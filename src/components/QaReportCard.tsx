...
export default function QaReportCard({ reports }: { reports: QaReport[] }) {
  ...
  return <div className={`rounded-2xl border p-5 ${warnings.length ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}>
    ...
    {warnings.length > 0 && <ul className="mt-3 space-y-1 text-sm text-amber-900">{warnings.map((warning, i) => <li key={`${warning.kind}-${i}`}>Section {warning.chunk} · {warning.label}: missing {warning.missing.join(", ")}</li>)}</ul>}
  </div>;
}
